...
payrollRouter.post("/payroll-employees", async (req: AuthedRequest, res) => {
  const { name, jobTitle, basicSalary, allowances, salaryExpenseAccountId, salaryPayableAccountId, isActive, notes } = req.body || {};
  ...
  try {
    ...
    await conn.query(
      `INSERT INTO payroll_employees (id, name, job_title, basic_salary, salary_expense_account_id, salary_payable_account_id, is_active, notes, created_at, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)`,
      [id, name, jobTitle || null, Number(basicSalary) || 0, salaryExpenseAccountId, salaryPayableAccountId, isActive === false ? 0 : 1, notes || null, req.authUser?.email || null]
    );
    for (const a of Array.isArray(allowances) ? allowances : []) {
      await conn.query(`INSERT INTO employee_allowances (id, employee_id, label, amount) VALUES (?, ?, ?, ?)`, [
        randomUUID(),
        id,
        a.label,
        Number(a.amount) || 0,
...
payrollRouter.put("/payroll-employees/:id", async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    await conn.query(
      ...
      [name, jobTitle || null, Number(basicSalary) || 0, salaryExpenseAccountId, salaryPayableAccountId, isActive ? 1 : 0, notes || null, req.params.id]
    );
...
payrollRouter.delete("/payroll-employees/:id", async (req, res) => {
  try {
    const pool = getPool();
    const [slips]: any = await pool.query(`SELECT COUNT(*) AS cnt FROM payslips WHERE employee_id = ?`, [req.params.id]);
    if (slips[0]?.cnt > 0) {
...
payrollRouter.get("/payroll-runs", async (_req, res) => {
  try {
    ...
    res.json(runs.map((r: any) => ({ ...r, paid: Boolean(r.paid), payableBreakdown: byRun.get(r.id) || [] })));
  } catch (err: any) {
...
payrollRouter.post("/payroll-runs", async (req: AuthedRequest, res) => {
  const { periodLabel, runDate, entryNumber, description, lines, totalGross, payableBreakdown, payslips } = req.body || {};
  if (
    !periodLabel ||
    !runDate ||
    !entryNumber ||
  ...
  try {
    ...
    await conn.query(
      ...
      [entryId, entryNumber, runDate, description, periodLabel, req.authUser?.email || null, req.authUser?.email || null]
    );
    let order = 0;
    ...
    for (const b of payableBreakdown) {
      await conn.query(`INSERT INTO payroll_run_payable_breakdown (id, payroll_run_id, account_id, amount) VALUES (?, ?, ?, ?)`, [
        randomUUID(),
        runId,
        b.accountId,
        Number(b.amount) || 0,
    ...
    for (const p of payslips) {
      ...
      await conn.query(
        `INSERT INTO payslips (id, payroll_run_id, employee_id, employee_name, period_label, basic_salary, gross_salary, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
        [slipId, runId, p.employeeId, p.employeeName, periodLabel, Number(p.basicSalary) || 0, Number(p.grossSalary) || 0]
      );
      for (const a of Array.isArray(p.allowances) ? p.allowances : []) {
        await conn.query(`INSERT INTO payslip_allowances (id, payslip_id, label, amount) VALUES (?, ?, ?, ?)`, [
          randomUUID(),
          slipId,
          a.label,
          Number(a.amount) || 0,
