...
export interface HearingsViewProps {
  ...
  setNotifyModal: (
    v: NotifyModalState | null | ((prev: NotifyModalState | null) => NotifyModalState | null),
  ) => void;
      ...
      {hearingSubTab === "deadlines" ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                <Hourglass className="text-amber-600" /> سجل مواعيد الطعون والتنبيهات الاستباقية (7
                أيام و 3 أيام)
              </h2>
              <p className="text-xs text-slate-500">
                نظام تلقائي يقوم بفحص المواعيد بانتظام وإرسال إشعارات استباقية للمحامي المسؤول عبر
                البريد والواتساب قبل الانقضاء بـ 7 أيام و 3 أيام
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  ...
                  if (
                    window.confirm(
                      `سيتم إيقاف كل التنبيهات الآلية الحالية (واتساب/إيميل) لجميع القضايا المفتوحة حالياً وعددها (${openCount})، عن طريق تعليمها كـ"لا حاجة لطعن". يمكنك لاحقاً إعادة أي قضية بعينها إلى المتابعة من قائمة "لا حاجة لطعن". هل تريد المتابعة؟`,
                    )
                  ) {
                    setDeadlines(
                      deadlines.map((x) =>
                        isDeadlineOpen(x.status) ? { ...x, status: "لا حاجة لطعن" } : x,
                      ),
                    );
              ...
              >
                ...
                {autoCheckStatus.isChecking
                  ? "جارٍ الفحص والإرسال..."
                  : "تشغيل فحص التنبيهات الآلية الآن"}
              </button>
              <button
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-emerald-900 flex items-center gap-1.5">
                  ...
                  محرك التنبيهات التلقائي الآلي: نَشِط ومفعّل (إشعارات البريد الإلكتروني والواتساب
                  قبل 7 أيام و 3 أيام)
                </p>
                <p className="text-[11px] text-emerald-700 mt-0.5">
                  {autoCheckStatus.message ||
                    `تم تجهيز خادم SMTP لإرسال التنبيهات فور وصول مهلة الطعن إلى 7 أيام أو 3 أيام مباشرة للمحامي الموكل`}
                </p>
              </div>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-4">
              ...
              <p className="text-2xl font-bold text-red-700">
                {
                  deadlines.filter(
                    (d) =>
                      ...
                      daysUntil(d.appealDeadlineDate) <= 3 &&
                      daysUntil(d.appealDeadlineDate) >= 0,
                  ).length
                }
              </p>
            </div>
              ...
              <p className="text-2xl font-bold text-amber-800">
                {
                  deadlines.filter(
                    (d) =>
                      isDeadlineOpen(d.status) &&
                      daysUntil(d.appealDeadlineDate) <= 7 &&
                      daysUntil(d.appealDeadlineDate) >= 0,
                  ).length
                }
              </p>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-emerald-800">
                {
                  deadlines.filter(
                    (d) => d.status === "تم قيد الطعن" || d.status === "تم تقديم الطعن",
                  ).length
                }
              </p>
            </div>
          ...
          <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
            ...
            >
              ⚠️ تحتاج تنبيه عاجل (أقل من 7 أيام) (
              {
                deadlines.filter(
                  (d) =>
                    isDeadlineOpen(d.status) &&
            ...
            {deadlines
              .filter((d) => {
                ...
                if (deadlineFilter === "urgent")
                  return isDeadlineOpen(d.status) && daysLeft <= 7 && daysLeft >= 0;
                if (deadlineFilter === "active") return d.status === "جارٍ حساب الميعاد";
                if (deadlineFilter === "done")
                  return d.status === "تم قيد الطعن" || d.status === "تم تقديم الطعن";
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-base text-slate-900">
                            {cs ? cs.number : `قضية رقم #${d.caseId}`}
                          </h3>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          الموكل: <b>{cs ? clientName(cs.clientId) : "—"}</b> • المحكمة:{" "}
                          {cs ? cs.court : "—"}
                        </p>
                      </div>
                      ...
                      <div className="text-left bg-white border border-slate-200 px-3 py-2 rounded-xl text-xs font-mono shadow-xs">
                        ...
                        <p className="font-bold text-slate-900 text-sm mt-0.5">
                          آخر موعد قاطع: {fmtDate(d.appealDeadlineDate)}
                        </p>
                        <div className="mt-1">
                          {d.status === "لا حاجة لطعن" ? (
                            <span className="text-emerald-700 font-bold px-2 py-0.5 rounded-md bg-emerald-50">
                              ✔️ لا حاجة لطعن — تم إيقاف التنبيهات
                            </span>
                          ) : !isDeadlineOpen(d.status) ? (
                            <span className="text-emerald-700 font-bold px-2 py-0.5 rounded-md bg-emerald-50">
                              ✔️ تم تقديم/قيد الطعن — لا حاجة لتنبيه
                            </span>
                          ) : daysLeft < 0 ? (
                            <span className="text-red-600 font-bold px-2 py-0.5 rounded-md bg-red-100">
                              ⚠️ انتهت المهلة القانونية
                            </span>
                          ) : daysLeft <= 3 ? (
                            <span className="text-red-700 font-black px-2 py-0.5 rounded-md bg-red-100 animate-pulse">
                              🚨 طارئ: متبقي {daysLeft} أيام فقط!
                            </span>
                          ) : daysLeft <= 7 ? (
                            <span className="text-amber-800 font-bold px-2 py-0.5 rounded-md bg-amber-100">
                              ⚠️ متبقي {daysLeft} أيام
                            </span>
                          ) : (
                            <span className="text-emerald-700 font-medium px-2 py-0.5 rounded-md bg-emerald-50">
                              متبقي {daysLeft} يومًا
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div>
                        <p className="text-slate-500 font-semibold mb-1 flex items-center justify-between">
                          <span className="flex items-center gap-1">
                            <UserCheck size={14} className="text-slate-700" /> المحامي المسؤول عن
                            الطعن:
                          </span>
                          <button
                        ...
                        <p className="font-bold text-slate-900 text-sm">
                          {d.assignedLawyerName || "المحامي سعود أحمد الشحي"}
                        </p>
                        <p className="text-slate-500 font-mono text-[11px] mt-0.5">
                          📧 {d.assignedLawyerEmail || "info@lawyersuood.com"} | 📱{" "}
                          {d.assignedLawyerPhone || "0501234567"}
                        </p>
                      </div>

                      <div className="space-y-1.5 border-r border-slate-200 pr-3">
                        <p className="text-slate-500 font-semibold flex items-center gap-1">
                          <Bell size={14} className="text-amber-600" /> حالة التنبيهات الاستباقية
                          الآلية:
                        </p>
                        <div className="flex flex-wrap items-center gap-1.5">
                          <span
                            className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${d.alert7DaysSent ? "bg-emerald-100 text-emerald-800 font-bold" : "bg-slate-200 text-slate-600"}`}
                          >
                            {d.alert7DaysSent
                              ? `🟢 إشعار 7 أيام: مرسل (${d.alert7DaysSentAt || "تم الإرسال"})`
                              : "⏳ إشعار 7 أيام: بانتظار الموعد"}
                          </span>
                          <span
                            className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${d.alert3DaysSent ? "bg-red-100 text-red-800 font-bold" : "bg-slate-200 text-slate-600"}`}
                          >
                            {d.alert3DaysSent
                              ? `🚨 إشعار 3 أيام: مرسل (${d.alert3DaysSentAt || "تم الإرسال"})`
                              : "⏳ إشعار 3 أيام: بانتظار الموعد"}
                          </span>
                        </div>
                      </div>
                    </div>
                          ...
                          d.status !== "لا حاجة لطعن" && (
                            <button
                              onClick={() =>
                                setDeadlines(
                                  deadlines.map((x) =>
                                    x.id === d.id ? { ...x, status: "تم قيد الطعن" } : x,
                                  ),
                                )
                              }
                          ...
                          d.status !== "لا حاجة لطعن" && (
                            <button
                              onClick={() => {
                                if (
                                  window.confirm(
                                    "هل أنت متأكد أن هذه القضية لا تحتاج لتقديم طعن؟ سيتم إيقاف كل التنبيهات الآلية (واتساب/إيميل) الخاصة بهذا الموعد نهائياً.",
                                  )
                                ) {
                                  setDeadlines(
                                    deadlines.map((x) =>
                                      x.id === d.id ? { ...x, status: "لا حاجة لطعن" } : x,
                                    ),
                                  );
                                }
                              }}
        ...
        <>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500">
                استخراج وتجهيز رول الجلسات اليومية والمزامنة مع تقويم Google Calendar وإرسال
                التنبيهات
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {googleUser && (
                  <span
                    ...
                    title={`متصل: ${googleUser.email}`}
                  />
                )}
              </button>
              <button
                onClick={() =>
                  setReport({ type: "roll", date: selectedRollDate, court: rollCourtFilter })
                }
              ...
              <button
                onClick={() => openModalWithCheck("hearing", "manageHearings")}
                className="flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-semibold text-white hover:bg-slate-700 shadow-sm transition cursor-pointer"
              >
                ...
                <button
                  onClick={() => {
                    const rollList = hearings.filter((h) => {
                      ...
                      const matchCourt =
                        rollCourtFilter === "الكل" || (cs && cs.court === rollCourtFilter);
                    ...
                    let rollTxt =
                      `🏛️ *جدول رول الجلسات لتاريخ ${selectedRollDate ? fmtDate(selectedRollDate) : "الكل"}*\n` +
            ...
            {hearings
              ...
              .map((h) => {
                ...
                return (
                  <div
                    key={h.id}
                    className="flex flex-wrap items-center justify-between gap-4 app-card p-4 hover:border-amber-300 transition"
                  >
                    <div className="flex items-center gap-4">
                      <div className="flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-xl bg-slate-900 text-amber-400 font-mono">
                        ...
                        <span className="text-xs">
                          {new Date(h.date).toLocaleDateString("ar-AE", { month: "short" })}
                        </span>
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {cs && (
                            <span className="text-xs text-slate-500 bg-stone-100 px-2 py-0.5 rounded-md font-medium">
                              {cs.court}
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-slate-500 mt-0.5">
                          الموكل: <b>{cs ? clientName(cs.clientId) : "—"}</b> • {h.type} • الساعة{" "}
                          {h.time} • {h.room}
                        </p>
                        {h.notes && (
                          <p className="mt-1 text-xs text-slate-600 bg-amber-50/60 px-2 py-1 rounded border border-amber-200/50">
                            المطلوب: {h.notes}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        title={
                          h.googleCalendarEventId
                            ? "تمت المزامنة مع تقويم Google"
                            : "مزامنة الجلسة مع تقويم Google"
                        }
                      >
                      ...
                      {h.done ? (
                        ...
                        <button
                          onClick={() => {
                            ...
                            setHearings(
                              hearings.map((x) => (x.id === h.id ? { ...x, done: true } : x)),
                            );
                          }}
                          ...
                          requestDelete({
                            ...
                            onConfirm: () => {
                              logAuditAction(
                                "DELETE",
                                "الجلسات",
                                `جلسة قضية: ${caseNo(h.caseId)}`,
                                `حذف جلسة ${h.type} بتأريخ ${h.date} للقضية ${caseNo(h.caseId)}`,
                                h.id,
                              );
