    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => {
              setForm({ startDate: todayISO(), endDate: todayISO() });
              setModal("leave-request");
            }}
          ...
          {isSuperAdmin && (
            <button
              onClick={() => {
                setForm({ actionDate: todayISO(), type: "تحقيق داخلي" });
                setModal("employee-disciplinary");
              }}
        ...
        <div className="app-card p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900 mt-1">
              {employees
                .reduce(
                  (sum, e) => sum + e.basicSalary + e.housingAllowance + e.transportAllowance,
                  0,
                )
                .toLocaleString()}{" "}
              د.إ
          ...
          <button
            ...
            className={`px-4 py-3 text-xs font-bold border-b-2 transition flex items-center gap-2 ${
              hrSubTab === "disciplinary"
                ? "border-red-600 text-red-700 bg-red-50/50 rounded-t-xl"
                : "border-transparent text-slate-500 hover:text-slate-800"
            }`}
          >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200">
            ...
            <span className="text-xs text-slate-500 font-medium">
              ...
              <span className="font-bold text-slate-900">
                {employees
                  .reduce(
                    (s, e) => s + e.basicSalary + e.housingAllowance + e.transportAllowance,
                    0,
                  )
                  .toLocaleString()}{" "}
                د.إ
                    ...
                    return (
                      <tr key={emp.id} className="hover:bg-amber-50/20 transition">
                        ...
                        <td className="p-3.5 text-slate-600">
                          ...
                          <p className="flex items-center gap-1 font-mono text-[11px] mt-0.5">
                        ...
                        <td className="p-3.5 text-slate-700">
                          <p className="font-mono text-[11px]">
                            <span className="text-slate-400">هوية:</span> {emp.emiratesId || "—"}
                          </p>
                          <p className="font-mono text-[11px] mt-0.5">
                            <span className="text-slate-400">جواز:</span>{" "}
                            {emp.passportNumber || "—"}
                          </p>
                          {emp.idExpiryDate && (
                            <div className="mt-1">
                              {daysToIdExpiry <= 60 ? (
                                ...
                                <span className="text-[10px] text-slate-400">
                                  تنتهي: {fmtDate(emp.idExpiryDate)}
                                </span>
                              )}
                            </div>
                          )}
                        ...
                        <td className="p-3.5">
                          <p className="font-bold text-slate-900">
                            {totalSalary.toLocaleString()} د.إ / شهرياً
                          </p>
                          <p className="text-[10px] text-slate-400">
                            أساسي: {emp.basicSalary.toLocaleString()} | سكن:{" "}
                            {emp.housingAllowance.toLocaleString()} | مواصلات:{" "}
            ...
            <table className="w-full min-w-[700px] text-right text-xs">
              <thead className="bg-slate-900 text-white font-bold">
                <tr>
                  ...
                  <th className="sticky left-0 z-10 p-3.5 text-center bg-slate-900 shadow-[-4px_0_6px_-4px_rgba(0,0,0,0.3)]">
                    اعتماد الإدارة
                  </th>
                </tr>
              </thead>
                ...
                {leaveRequests.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-400">
                      لا توجد طلبات إجازات مسجلة حالياً
                    </td>
                  </tr>
                ) : (
                  leaveRequests.map((leave) => (
                    <tr key={leave.id} className="hover:bg-slate-50 group">
                      ...
                      <td className="p-3.5 text-slate-600 max-w-xs truncate" title={leave.reason}>
                        {leave.reason || "—"}
                      </td>
                      <td className="p-3.5 text-center">
                        {leave.status === "PENDING" && (
                          <span className="bg-amber-100 text-amber-800 px-2.5 py-1 rounded-full font-bold text-[11px]">
                            قيد الانتظار
                          </span>
                        )}
                        ...
                        {leave.status === "REJECTED" && (
                          <span className="bg-red-100 text-red-800 px-2.5 py-1 rounded-full font-bold text-[11px]">
                            مرفوضة
                          </span>
                        )}
                      </td>
                ...
                {employeeExpenses.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400">
                      لا توجد مطالبات مصروفات حالياً
                    </td>
                  </tr>
                ) : (
                    ...
                    return (
                      <tr key={exp.id} className="hover:bg-slate-50 group">
                        ...
                        <td className="p-3.5 font-medium">
                          ...
                          {exp.category === "SUPPLIES" && (
                            <span className="bg-amber-100 text-amber-800 font-bold px-2 py-0.5 rounded text-[11px]">
                              أدوات ومستلزمات مكتب
                            </span>
                          )}
                          {exp.category === "OTHER" && (
                            <span className="bg-slate-100 text-slate-700 font-bold px-2 py-0.5 rounded text-[11px]">
      ...
      {hrSubTab === "disciplinary" && isSuperAdmin && (
        <div className="space-y-4">
          <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl p-3 text-[11px] text-red-800">
            ...
            <p>
              هذا السجل سري ومقتصر على الحساب الرئيسي فقط، ولا يظهر لبقية الكادر أو في أي قسم عام
              آخر بالنظام.
            </p>
          </div>
            ...
            <div className="space-y-3">
              {[...disciplinaryActions]
                ...
                .map((act) => (
                  <div
                    key={act.id}
                    className="bg-white rounded-2xl border border-slate-200 shadow-xs p-4"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className="text-[11px] text-slate-400">
                        تاريخ الإجراء: {fmtDate(act.actionDate)}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-100">
                      <span className="text-[10px] text-slate-400">
                        بواسطة: {act.createdBy || "—"} | {fmtDate(act.createdAt)}
                      </span>
                      <button
