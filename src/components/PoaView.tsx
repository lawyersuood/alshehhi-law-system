      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            توكيلات الكاتب العدل وتنبيهات الانتهاء الصادرة من كاتب العدل
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Sparkles size={16} className="text-amber-200 animate-pulse" /> ارفاق وكالة PDF (سحب
            البيانات آلياً)
          </button>
          <button
            onClick={() => openModalWithCheck("poa", "manageDocs")}
          ...
          {poas.map((p) => {
            ...
            return (
              <div key={p.id} className="app-card p-5 space-y-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h3 className="font-bold text-slate-900">
                      {p.number} — {clientName(p.clientId)}
                    </h3>
                    <p className="text-xs text-slate-500">
                      {p.issuer} | تاريخ الإصدار: {fmtDate(p.issue)}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {daysLeft < 0 ? (
                      <Badge className="bg-red-100 text-red-800 font-black border border-red-200 px-3 py-1">
                        منتهية
                      </Badge>
                    ) : daysLeft <= 60 ? (
                      <Badge className="bg-amber-100 text-amber-800 font-bold">
                        تنتهي خلال {daysLeft} يومًا
                      </Badge>
                    ) : (
                      <Badge className="bg-emerald-100 text-emerald-700">
                        سارية ({daysLeft} يومًا)
                      </Badge>
                    )}
                    <button
                        ...
                        requestDelete({
                          ...
                          onConfirm: () => {
                            logAuditAction(
                              ...
                              "الوكالات",
                              `وكالة: ${p.number}`,
                              `حذف الوكالة رقم ${p.number} للموكل ${clientName(p.clientId)}`,
                              p.id,
                            );
                ...
                <p className="text-xs text-slate-700 bg-stone-50 p-2 rounded-lg border border-stone-200 font-medium">
                  صلاحيات الوكالة: {p.scope}
                </p>
              </div>
            );
          })}
