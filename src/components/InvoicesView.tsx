...
export interface InvoicesViewProps {
  ...
  setInvoiceSubTab: (
    tab: "invoices" | "agreements" | "payments" | "time" | "trust" | "expenses",
  ) => void;
      ...
      {!canViewFinancials ? (
        <div className="p-8 bg-white rounded-2xl border border-slate-200 text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            حسابك الحالي ({currentUser.roleTitle}) لا يمتلك صلاحية الوصول إلى النظام المالي
            والفواتير. يرجى التبديل إلى حساب المحاسب أو مدير النظام.
          </p>
        </div>
      ) : (
        <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    تسجيل وتخصيص الدفعات المقبوضة لاتفاقيات الأتعاب أو ترحيلها للرصيد المعلّق
                  </p>
                </div>
                <button
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
                  <p className="text-xs text-emerald-800 font-semibold">
                    الدفعات المخصصة للاتفاقيات
                  </p>
                  <p className="text-2xl font-bold text-emerald-800 font-mono">
                    {fmtAED(
                      payments
                        .filter(
                          (p) => p.feeAgreementId !== "unallocated" && p.feeAgreementId != null,
                        )
                ...
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
                  <p className="text-xs text-amber-800 font-semibold">
                    الأرصدة المعلقة (دفعات غير مخصصة)
                  </p>
                ...
                <table className="w-full min-w-[700px] text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100/80">
                    {payments.map((p) => {
                      ...
                      const agreement = !isUnallocated
                        ? feeAgreements.find((a) => a.id === p.feeAgreementId)
                        : null;
                                ...
                                requestDelete({
                                  ...
                                  onConfirm: () => {
                                    logAuditAction(
                                      "DELETE",
                                      "المالية والمقبوضات",
                                      `سند قبض: ${p.referenceNo}`,
                                      `حذف سند القبض ${p.referenceNo} بمبلغ ${p.amount} درهم للموكل ${client?.name || ""}`,
                                      p.id,
                                    );
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    متابعة قيم العقود، التحصيلات، المتبقي على كل اتفاقية، والربط مع الدفعات
                  </p>
                </div>
                <button
                  onClick={() => openModalWithCheck("payment", "manageInvoices")}
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
                  <p className="text-xs text-amber-800 font-semibold">
                    إجمالي المتبقي على الاتفاقيات النشطة
                  </p>
                  <p className="text-2xl font-bold text-amber-800 font-mono">
                    {fmtAED(
                      feeAgreements
                        .filter((a) => a.status === "نشطة")
                        .reduce((sum, a) => sum + getRemainingForAgreement(a), 0),
                    )}
                  </p>
                </div>
              </div>
              ...
              {feeAgreements.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-slate-300 bg-white space-y-3">
                  ...
                  <p className="text-sm font-bold text-slate-800">
                    لا توجد اتفاقيات أتعاب مسجلة حالياً
                  </p>
                  <p className="text-xs text-slate-500 max-w-sm">
                    يمكنك تسجيل اتفاقية أتعاب جديدة لأحد الموكلين لمتابعة قيمتها والمحصّل والمتبقي
                    منها.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {feeAgreements.map((agr) => {
                    ...
                    return (
                      <div
                        key={agr.id}
                        className="app-card p-5 space-y-4 hover:border-amber-300 transition"
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-3">
                          <div>
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                              الموكل: <b>{client?.name || "—"}</b> • تاريخ العقد:{" "}
                              {fmtDate(agr.date)}
                            </p>
                          </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => {
                                ...
                                const matchingOA = officeAgreements.find(
                                  (oa) =>
                                    oa.feeAgreementId === agr.id ||
                                    oa.agreementNumber === agr.agreementNumber,
                                );
                                if (matchingOA) {
                        ...
                        <div className="grid gap-3 sm:grid-cols-3 bg-stone-50 p-3.5 rounded-xl border border-stone-200 text-xs">
                          ...
                          <div>
                            ...
                            <span
                              className={`font-bold text-sm font-mono ${remaining > 0 ? "text-amber-800" : "text-slate-400"}`}
                            >
                              {fmtAED(remaining)}
                            </span>
                          </div>
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    مطالبات الأتعاب بتنسيق الهيئة الاتحادية للضرائب FTA (TRN: {FIRM_TRN})
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={() => openModalWithCheck("invoice", "manageInvoices")}
                    className="flex items-center gap-2 rounded-xl bg-[#0D382B] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#124d40] transition-colors shadow-[0_8px_20px_-8px_rgb(13,56,43,0.5)]"
                  >
                    <Plus size={16} /> إصدار فاتورة ضريبية
                  </button>
                </div>
              </div>
              <div className="overflow-x-auto custom-scrollbar app-card">
                    ...
                    {invoices.map((inv) => {
                      ...
                      return (
                        <tr key={inv.id} className="hover:bg-amber-50/50 group">
                          ...
                          <td className="px-4 py-3">
                            <Badge className={invColor(evs)}>{evs}</Badge>
                          </td>
                          <td className="sticky left-0 z-10 bg-white group-hover:bg-amber-50/50 px-4 py-3 text-center shadow-[-4px_0_6px_-4px_rgba(0,0,0,0.08)]">
                            <div className="flex items-center justify-center gap-1">
                              {inv.status === "مسودة" && (
                                <button
                                  onClick={() => setInvoiceStatus(inv.id, "مرسلة")}
                                  className="p-1.5 text-sky-600 hover:text-sky-800 rounded-lg hover:bg-sky-50 flex items-center gap-1 text-xs font-semibold"
                              ...
                              {(inv.status === "مرسلة" || evs === "متأخرة") && (
                                <button
                                  onClick={() => setInvoiceStatus(inv.id, "مدفوعة")}
                                  className="p-1.5 text-emerald-700 hover:text-emerald-900 rounded-lg hover:bg-emerald-50 flex items-center gap-1 text-xs font-bold"
                                  title="تحديد الفاتورة كمُسدَّدة بالكامل"
                                >
                                  <CheckCircle2 size={14} /> تحديد كمسددة
                                </button>
                              )}
                              <button
                                onClick={() => openNotificationComposer("تذكير فاتورة", inv)}
                                className="p-1.5 text-emerald-600 hover:text-emerald-800 rounded-lg hover:bg-emerald-50 flex items-center gap-1 text-xs font-semibold"
                                title="تذكير الموكل بسداد الفاتورة"
                              >
                                <Send size={14} /> تذكير بالواتساب/الإيميل
                              </button>
                              <button
                                onClick={() =>
                                  setReport({ type: "tax-invoice", invoiceId: inv.id })
                                }
                                className="p-1.5 text-amber-700 hover:text-amber-900 rounded-lg hover:bg-amber-50 flex items-center gap-1 text-xs font-bold"
                                title="عرض وطباعة فاتورة FTA الرسمية"
                              >
                                  ...
                                  requestDelete({
                                    ...
                                    onConfirm: () => {
                                      logAuditAction(
                                        ...
                                        "الفواتير",
                                        `فاتورة: ${inv.number}`,
                                        `حذف الفاتورة ${inv.number} للموكل ${clientName(inv.clientId)} بمبلغ ${total} درهم`,
                                        inv.id,
                                      );
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    تسجيل الوقت المستغرق بكل قضية وتحويله تلقائياً لفاتورة أتعاب بالساعة
                  </p>
                </div>
                <button
                  onClick={() => openModalWithCheck("time")}
                  className="flex items-center gap-2 rounded-xl bg-[#0D382B] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#124d40] transition-colors shadow-[0_8px_20px_-8px_rgb(13,56,43,0.5)]"
                >
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="app-card p-4">
                  ...
                  <p className="text-2xl font-bold text-slate-900">
                    {timeLogs.reduce((acc, t) => acc + t.hours, 0)} ساعة
                  </p>
                </div>
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
                  ...
                  <p className="text-2xl font-bold text-emerald-800">
                    {timeLogs.length > 0
                      ? Math.round(
                          (timeLogs.filter((t) => t.billed).length / timeLogs.length) * 100,
                        )
                      : 0}
                    %
                  </p>
              ...
              <div className="space-y-3">
                {timeLogs.map((log) => {
                  ...
                  return (
                    <div
                      key={log.id}
                      className="flex flex-wrap items-center justify-between gap-4 app-card p-4"
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-500 bg-stone-100 px-2 py-0.5 rounded font-medium">
                            {log.lawyerName}
                          </span>
                          {log.billed ? (
                            <Badge className="bg-emerald-100 text-emerald-700">مفوترة</Badge>
                          ) : (
                            <Badge className="bg-amber-100 text-amber-800">غير مفوترة</Badge>
                          )}
                        </div>
                        <p className="text-xs text-slate-600">{log.description}</p>
                        <p className="text-[11px] text-slate-400">
                          التاريخ: {fmtDate(log.date)} • الموكل:{" "}
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-left font-mono">
                          <p className="text-xs text-slate-500">
                            {log.hours} ساعة × {log.hourlyRate} د.إ
                          </p>
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    تتبع مبالغ أمانات رسوم المحاكم والخبراء منفصلة تماماً عن أتعاب المكتب
                  </p>
                </div>
                <button
                  onClick={() => openModalWithCheck("trust")}
                  className="flex items-center gap-2 rounded-xl bg-[#0D382B] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#124d40] transition-colors shadow-[0_8px_20px_-8px_rgb(13,56,43,0.5)]"
                >
              ...
              {(() => {
                const clientsWithTrust = clients.filter((c) =>
                  trustTransactions.some((t) => t.clientId === c.id),
                );
                return clientsWithTrust.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-slate-300 bg-white space-y-3">
                    ...
                    <p className="text-sm font-bold text-slate-800">
                      لا توجد معاملات أمانة مسجلة حالياً
                    </p>
                    <p className="text-xs text-slate-500 max-w-sm">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {clientsWithTrust.map((c) => {
                      ...
                      const deposits = cTx
                        .filter((t) => t.type === "إيداع أمانة")
                        .reduce((acc, t) => acc + t.amount, 0);
                      const withdrawals = cTx
                        .filter((t) => t.type === "صرف أمانة")
                        .reduce((acc, t) => acc + t.amount, 0);
                    ...
                    <tbody className="divide-y divide-slate-100/80">
                      {trustTransactions.map((tx) => (
                        <tr key={tx.id} className="hover:bg-amber-50/50">
                          ...
                          <td
                            className={`px-4 py-3 font-mono font-bold ${tx.type === "إيداع أمانة" ? "text-emerald-700" : "text-amber-700"}`}
                          >
                            {tx.type === "إيداع أمانة" ? "+" : "-"}
                            {fmtAED(tx.amount)}
                          </td>
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    متابعة تكاليف الروم القضائية والخبرة والترجمة وحساب الهامش الصافي لكل قضية
                  </p>
                </div>
                <button
                  onClick={() => openModalWithCheck("expense")}
                  className="flex items-center gap-2 rounded-xl bg-[#0D382B] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#124d40] transition-colors shadow-[0_8px_20px_-8px_rgb(13,56,43,0.5)]"
                >
              ...
              {(() => {
                const casesWithActivity = cases.filter(
                  (cs) =>
                ...
                return casesWithActivity.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-12 text-center rounded-2xl border border-dashed border-slate-300 bg-white space-y-3">
                    ...
                    <p className="text-sm font-bold text-slate-800">
                      لا توجد بيانات مالية على القضايا حتى الآن
                    </p>
                    <p className="text-xs text-slate-500 max-w-sm">
                      بمجرد تسجيل فاتورة أو ساعات عمل أو مصروف على إحدى القضايا، سيظهر هنا تقرير
                      ربحيتها.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {casesWithActivity.map((cs) => {
                      ...
                      const totalTimeCost = csTime.reduce(
                        (acc, t) => acc + t.hours * t.hourlyRate,
                        0,
                      );
                          ...
                          <div className="flex flex-wrap items-center justify-between gap-3">
                            <div>
                              <h3 className="font-bold text-slate-900 text-base">
                                {cs.number} — {clientName(cs.clientId)}
                              </h3>
                              <p className="text-xs text-slate-500">
                                {cs.court} • الموضوع: {cs.subject}
                              </p>
                            </div>
                            <button
