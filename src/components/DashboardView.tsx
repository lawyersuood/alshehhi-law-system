...
export default function DashboardView({
  ...
  const [dashboardInvoiceChartMode, setDashboardInvoiceChartMode] = useState<"amount" | "count">(
    "amount",
  );
  const [dashboardTaskChartMode, setDashboardTaskChartMode] = useState<"status" | "priority">(
    "status",
  );
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-500 text-slate-950 font-bold shadow-sm">
                ...
                <span className="absolute -top-1 -right-1 flex h-3.5 w-3.5">
                  ...
                  <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-red-600 text-[9px] text-white font-bold items-center justify-center">
                    !
                  </span>
                </span>
              </div>
              <div>
                ...
                <p className="text-xs text-slate-600 mt-0.5">
                  يوجد <span className="font-bold text-red-700">{urgentTasks24h.length}</span> مهام
                  تستحق المتابعة والإنجاز الفوري أو قاربت مهلتها على الانتهاء
                </p>
              </div>
            </div>
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${TASK_PRIORITY[t.priority] || "bg-slate-100 text-slate-700"}`}
                      >
                        {t.priority}
                      </span>
                    </div>
                    ...
                    <div className="mt-2 space-y-1 text-xs text-slate-600">
                      ...
                      <div className="flex items-center gap-1.5 text-slate-600">
                        ...
                        <span>
                          المكلف: <strong className="text-slate-800">{t.assignee}</strong>
                        </span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="mr-1 text-slate-700">
                لا توجد مهام مستحقة خلال الـ 24 ساعة القادمة. جميع المهام ضمن جدولها الطبيعي.
              </span>
            </div>
          </div>
      ...
      {/* بطاقات المؤشرات */}
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 ${canViewFinancials ? "lg:grid-cols-4" : "lg:grid-cols-3"} gap-3 sm:gap-4`}
      >
        {[
          {
            label: "قضايا نشطة",
          ...
          .map((k: any) => (
            <div key={k.label} className="app-card p-5">
              <div
                className={`mb-3.5 flex h-11 w-11 items-center justify-center rounded-2xl ${k.tone}`}
              >
                <k.icon size={20} />
              </div>
              <p
      ...
      <div
        className={`grid gap-4.5 ${canViewFinancials ? "grid-cols-1 md:grid-cols-2 xl:grid-cols-3" : "grid-cols-1 lg:grid-cols-2"}`}
      >
                  ...
                  {dashboardCaseChartMode === "bar" ? (
                    ...
                    >
                      ...
                      <Tooltip
                        ...
                        formatter={(value: any) => [
                          `${value} قضية (${Math.round((Number(value) / (cases.length || 1)) * 100)}%)`,
                          "عدد القضايا",
                        ]}
                      />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">
                      نسبة السداد:{" "}
                      <b className="text-emerald-700 font-bold">{invoiceMetrics.collectionRate}%</b>
                    </p>
                  </div>
                </div>
                <div className="flex items-center bg-slate-100/70 p-1 rounded-xl border border-slate-200/70">
                      ...
                      <Pie
                        data={
                          dashboardInvoiceChartMode === "amount"
                            ? invoiceMetrics.summaryByAmount
                            : invoiceMetrics.summaryByCount
                        }
                        dataKey="value"
                        nameKey="name"
                        innerRadius={45}
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-3 gap-1.5 text-center">
              <div className="p-1.5 rounded-lg bg-emerald-50 border border-emerald-200">
                ...
                <p className="text-xs font-bold text-emerald-900 truncate">
                  {fmtAED(invoiceMetrics.paidAmount)}
                </p>
              </div>
              <div className="p-1.5 rounded-lg bg-sky-50 border border-sky-200">
                ...
                <p className="text-xs font-bold text-sky-900 truncate">
                  {fmtAED(invoiceMetrics.sentDueAmount)}
                </p>
              </div>
              <div className="p-1.5 rounded-lg bg-red-50 border border-red-200">
                ...
                <p className="text-xs font-bold text-red-900 truncate">
                  {fmtAED(invoiceMetrics.overdueAmount)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="app-card p-5 flex flex-col justify-between">
            ...
            <div className="flex items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">
                    الإنجاز:{" "}
                    <b className="text-indigo-700 font-bold">{tasksMetrics.completionRate}%</b> (
                    {tasksMetrics.completed} من {tasksMetrics.total})
                  </p>
                </div>
              </div>
              <div className="flex items-center bg-slate-100/70 p-1 rounded-xl border border-slate-200/70">
              ...
              {tasksMetrics.total > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  {dashboardTaskChartMode === "status" ? (
                    ...
                    <BarChart
                      data={tasksMetrics.priorityChartData}
          ...
          <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
            ...
            <span
              className={`flex items-center gap-1 font-bold px-2 py-0.5 rounded-md border ${tasksMetrics.overdue + tasksMetrics.dueToday > 0 ? "text-red-700 bg-red-50 border-red-200 animate-pulse" : "text-slate-600 bg-slate-50 border-slate-200"}`}
            >
      ...
      <div className="rounded-2xl border border-amber-300 bg-amber-50/50 p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base">
                المهام المستحقة اليوم أو المتأخرة ({urgentTodayOrOverdueTasks.length})
              </h3>
              <p className="text-xs text-slate-600">
                المهام التي تنتهي صلاحيتها اليوم أو التي تجاوزت موعد الاستحقاق مع إمكانية الإنجاز
                الفوري
              </p>
            </div>
          </div>
          ...
          >
            عرض كافة المهام <ChevronLeft size={14} />
          </button>
        </div>
        <div className="space-y-3">
          {urgentTodayOrOverdueTasks.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className="flex items-center justify-between gap-3 rounded-xl bg-white p-4 border border-amber-200/80 shadow-2xs"
              >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 flex items-center gap-2 flex-wrap mt-0.5">
                      ...
                      <span>
                        ...
                        <b
                          className={
                            isOverdue ? "text-red-600 font-bold" : "text-amber-700 font-bold"
                          }
                        >
                          {fmtDate(t.due)} (
                          {isOverdue ? `متأخرة بـ ${Math.abs(dLeft)} أيام` : "اليوم"})
                        </b>
                      </span>
                    </p>
                  </div>
          ...
          {urgentTodayOrOverdueTasks.length === 0 && (
            <div className="rounded-xl bg-white p-6 text-center text-slate-500 border border-slate-200">
              ...
              <p className="font-bold text-slate-700 text-sm">
                ممتاز! لا توجد مهام متأخرة أو مستحقة اليوم
              </p>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="app-card p-5">
          ...
          <div className="space-y-3">
            ...
            {upcoming.length === 0 && (
              <p className="py-6 text-center text-sm text-slate-400">لا توجد جلسات قادمة</p>
            )}
          </div>
        </div>
        {/* المهام المعلقة */}
              ...
              .map((t) => (
                <div key={t.id} className="flex items-center gap-3 rounded-xl bg-stone-50 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      الاستحقاق: {fmtDate(t.due)} — المكلف: {t.assignee}
                    </p>
                  </div>
                  <Badge className={TASK_PRIORITY[t.priority]}>{t.priority}</Badge>
                </div>
