...
export const normalizePhoneDigits = (phone: string = ""): string => {
  if (!phone) return "";
  return phone.replace(/[٠-٩]/g, (d) => "٠١٢٣٤٥٦٧٨٩".indexOf(d).toString()).replace(/[^0-9]/g, "");
};
...
export const addDays = (d: number) => {
  const t = new Date();
  t.setDate(t.getDate() + d);
  return toDubaiISODate(t);
};

export const addDaysFrom = (baseDate: string, d: number) => {
...
export const isDeadlineOpen = (status: string) =>
  status !== "تم قيد الطعن" && status !== "تم تقديم الطعن" && status !== "لا حاجة لطعن";
...
export const getCaseStage = (c: {
  ...
  if (txt.includes("تمييز") || txt.includes("نقض") || txt.includes("العليا"))
    return "التمييز / النقض";
  if (txt.includes("تنفيذ")) return "التنفيذ";
  if (
    txt.includes("لجنة") ||
    txt.includes("منازعات") ||
    txt.includes("توفيق") ||
    txt.includes("إيجاري")
  )
    return "لجان فض المنازعات";
  return "الابتدائية";
};
...
export function isDemoCase(c: any): boolean {
  ...
  const opp = String(
    Array.isArray(c.opponents) ? c.opponents.join(" ") : c.opponent || "",
  ).toLowerCase();
...
export function sanitizeCase(c: CaseItem): CaseItem {
  ...
  const opponents = rawOpponents
    .map((o) => (o || "").trim())
    .filter((o) => o && !PLACEHOLDER_OPPONENTS.has(o));

  let judge = c.judge || "";
  const fee = typeof c.fee === "number" && !isNaN(c.fee) ? c.fee : 0; // الإبقاء على الأتعاب الفعلية المدخلة؛ صفر فقط إن لم تكن مُدخلة أصلاً
  const openDate = c.openDate || "";
...
export function deduplicateCases(caseList: CaseItem[]): CaseItem[] {
  ...
  for (const raw of caseList) {
    ...
    const compKey = `${c.clientId}_${String(c.court || "")
      .trim()
      .toLowerCase()}_${String(c.subject || "")
      .trim()
      .toLowerCase()}`;

    if (
      normNum &&
      seenNumberKeys.has(normNum) &&
      seenNumberKeys.get(normNum)!.clientId === c.clientId
    ) {
      const existing = seenNumberKeys.get(normNum)!;
      if (
        (!existing.opponents || existing.opponents.length === 0) &&
        c.opponents &&
        c.opponents.length > 0
      )
        existing.opponents = c.opponents;
      if (!existing.judge && c.judge) existing.judge = c.judge;
      if (
        (!existing.court || existing.court === "محاكم دبي") &&
        c.court &&
        c.court !== "محاكم دبي"
      ) {
