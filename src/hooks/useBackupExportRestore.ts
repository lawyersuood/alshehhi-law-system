...
import type {
  Client, CaseItem, Hearing, TaskItem, Invoice, DocItem, PoaItem, KycItem, CourtContact,
  OfficeAgreement, Employee, LeaveRequest, EmployeeExpense, EmployeeDisciplinaryAction,
  AuditLogEntry, LegalPrecedent, InternalPolicy, UserItem, FeeAgreement, PaymentReceipt,
} from "../domain/types";
...
export interface BackupExportRestoreDeps {
  ...
  clients: Client[]; setClients: (v: Client[]) => void;
  cases: CaseItem[]; setCases: (v: CaseItem[]) => void;
  hearings: Hearing[]; setHearings: (v: Hearing[]) => void;
  tasks: TaskItem[]; setTasks: (v: TaskItem[]) => void;
  invoices: Invoice[]; setInvoices: (v: Invoice[]) => void;
  docs: DocItem[]; setDocs: (v: DocItem[]) => void;
  poas: PoaItem[]; setPoas: (v: PoaItem[]) => void;
  kyc: KycItem[]; setKyc: (v: KycItem[]) => void;
  courtContacts: CourtContact[]; setCourtContacts: (v: CourtContact[]) => void;
  officeAgreements: OfficeAgreement[]; setOfficeAgreements: (v: OfficeAgreement[]) => void;
  employees: Employee[]; setEmployees: (v: Employee[]) => void;
  leaveRequests: LeaveRequest[]; setLeaveRequests: (v: LeaveRequest[]) => void;
  employeeExpenses: EmployeeExpense[]; setEmployeeExpenses: (v: EmployeeExpense[]) => void;
  disciplinaryActions: EmployeeDisciplinaryAction[]; setDisciplinaryActions: (v: EmployeeDisciplinaryAction[]) => void;
  auditLogs: AuditLogEntry[]; setAuditLogs: (v: AuditLogEntry[]) => void;
  precedents: LegalPrecedent[]; setPrecedents: (v: LegalPrecedent[]) => void;
  policies: InternalPolicy[]; setPolicies: (v: InternalPolicy[]) => void;
  users: UserItem[]; setUsers: (v: UserItem[]) => void;
  feeAgreements: FeeAgreement[]; setFeeAgreements: (v: FeeAgreement[]) => void;
  payments: PaymentReceipt[]; setPayments: (v: PaymentReceipt[]) => void;
  consultationBookings: BookingRecord[]; setConsultationBookings: (v: BookingRecord[]) => void;
  consultationSettings: ConsultationSettings; setConsultationSettings: (v: ConsultationSettings) => void;
  waChats: WaChat[]; setWaChats: (v: WaChat[]) => void;
...
export function useBackupExportRestore(deps: BackupExportRestoreDeps) {
  const handleExportBackup = async () => {
    const {
      currentUser, clients, cases, hearings, tasks, invoices, docs, poas, kyc, courtContacts,
      officeAgreements, employees, leaveRequests, employeeExpenses, disciplinaryActions, auditLogs,
      precedents, policies, users, feeAgreements, payments, consultationBookings, consultationSettings,
      waChats, logAuditAction,
    } = deps;
  ...
  const executeRestore = () => {
    const {
      restorePreview, setClients, setCases, setHearings, setTasks, setInvoices, setDocs, setPoas,
      setKyc, setCourtContacts, setOfficeAgreements, setEmployees, setLeaveRequests,
      setEmployeeExpenses, setDisciplinaryActions, setAuditLogs, setPrecedents, setPolicies, setUsers,
      setFeeAgreements, setPayments, setConsultationBookings, setConsultationSettings, setWaChats,
      logAuditAction, setRestoreSuccessMsg, setRestorePreview, setRestoreError,
    } = deps;
