...
export function useBillingRecords() {
  const [timeLogs, setTimeLogs] = useSyncedTable<TimeLog>("firm_time_logs", "time_logs", () => loadStorage("firm_time_logs", seedTimeLogs));
  const [caseExpenses, setCaseExpenses] = useSyncedTable<CaseExpense>("firm_case_expenses", "case_expenses", () => loadStorage("firm_case_expenses", seedCaseExpenses));
  const [trustTransactions, setTrustTransactions] = useSyncedTable<TrustTransaction>("firm_trust_transactions", "trust_transactions", () => loadStorage("firm_trust_transactions", seedTrustTransactions));
  const [installments, setInstallments] = useSyncedTable<InvoiceInstallment>("firm_installments", "invoice_installments", () => loadStorage("firm_installments", seedInstallments));
  const [strReports, setStrReports] = useSyncedTable<StrReport>("firm_str_reports", "str_reports", () => loadStorage("firm_str_reports", seedStrReports));
