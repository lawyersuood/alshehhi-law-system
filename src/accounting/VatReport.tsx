...
export default function VatReport({
  ...
  const vatOutputAccount = useMemo(
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-500">
          مرجع تمهيدي (ضريبة المخرجات − ضريبة المدخلات) لا يغني عن مراجعة الإقرار الضريبي الفعلي قبل
          تقديمه للهيئة الاتحادية للضرائب
        </p>
      </div>
      ...
      {(!vatOutputAccount || !vatInputAccount) && (
        <div className="flex items-center gap-2 rounded-xl bg-[#0D382B]/[0.05] border border-[#0D382B]/15 px-4 py-3 text-xs text-[#0D382B]">
          ...
          تنبيه: لم يتم العثور على حساب ضريبة المخرجات ({VAT_OUTPUT_ACCOUNT_CODE}) أو ضريبة المدخلات
          ({VAT_INPUT_ACCOUNT_CODE}) في شجرة الحسابات — القيم المعروضة قد تكون غير مكتملة.
        </div>
      )}
          ...
          <tbody>
            ...
            <tr className="border-b border-slate-50">
              <td className="px-4 py-3 text-slate-800">
                ضريبة المدخلات القابلة للاسترداد (على المشتريات المعتمدة)
              </td>
          ...
          <tfoot>
            <tr className="border-t border-slate-200 font-bold text-slate-900">
              <td className="px-4 py-3">
                صافي الضريبة {netVat >= 0 ? "المستحقة للهيئة" : "القابلة للاسترداد منها"}
              </td>
      ...
      <div className="rounded-2xl px-5 py-4 flex items-center gap-2 bg-slate-50 text-slate-600 text-xs">
        <Percent size={15} />
        هذا التقرير يُحتسب آلياً من قيود الاعتماد على فواتير المبيعات والمشتريات ضمن هذا النظام
        المحاسبي المعزول فقط، ولا يشمل أي بيانات ضريبية من خارجه.
      </div>
    </div>
  );
}
