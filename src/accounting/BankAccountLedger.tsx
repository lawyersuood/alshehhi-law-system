...
export default function BankAccountLedger({
  ...
  const otherAccounts = useMemo(
    () =>
      accounts
        .filter((a) => a.isActive && !a.isGroup && a.id !== bankAccount.linkedAccountId)
  ...
  const saveTransaction = () => {
    ...
    const newEntry: JournalEntry = {
      ...
      lines: isDeposit
        ? [
            {
              id: `l-${journalEntryId}-1`,
              accountId: bankAccount.linkedAccountId,
              debit: amount,
              credit: 0,
            },
            {
              id: `l-${journalEntryId}-2`,
              accountId: draft.contraAccountId,
              debit: 0,
              credit: amount,
            },
          ]
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            ...
            <span className="font-mono font-semibold text-slate-800">
              {fmtMoney(currentBalance, bankAccount.currency)}
            </span>
          </p>
        </div>
        {canRecord && (
              ...
              {rows.map((t) => (
                <tr key={t.id} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-xs text-slate-500">
                    {accountLabel(t.contraAccountId)}
                  </td>
                  <td
                    className={`px-4 py-2.5 font-mono font-semibold ${t.type === "deposit" ? "text-emerald-700" : "text-rose-700"}`}
                  >
                    {t.type === "deposit" ? "+" : "-"}
                    {fmtMoney(t.amount, bankAccount.currency)}
                  </td>
                  <td className="px-4 py-2.5 font-mono text-slate-900">
                    {fmtMoney(t.balance, bankAccount.currency)}
                  </td>
                  {canDelete && (
                    <td className="px-4 py-2.5">
                      <button
        ...
        >
          ...
          >
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 mb-1 block">
                {draft.type === "deposit"
                  ? "مصدر الإيداع (الحساب المقابل)"
                  : "وجهة الصرف (الحساب المقابل)"}
              </label>
              <select
                value={draft.contraAccountId}
                onChange={(e) => setDraft({ ...draft, contraAccountId: e.target.value })}
                className={inputCls}
              >
            ...
            <p className="text-[11px] text-slate-400">
              سيتم إنشاء قيد يومية مرحّل تلقائياً بهذه الحركة، متوازن بين حساب البنك والحساب المقابل
              المختار.
            </p>
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-600">
              سيتم حذف هذه الحركة وقيد اليومية المرتبط بها معاً. هل أنت متأكد؟ لا يمكن التراجع عن
              هذا الإجراء.
            </p>
            <div className="flex gap-2 justify-end pt-2">
              <button
