...
export default function PayrollRuns({
  ...
  const previewTotal = useMemo(() => activeEmployees.reduce((sum, e) => sum + grossSalary(e), 0), [activeEmployees]);
  ...
  const runPayroll = () => {
    ...
    for (const emp of activeEmployees) {
      ...
      expenseByAccount.set(emp.salaryExpenseAccountId, (expenseByAccount.get(emp.salaryExpenseAccountId) || 0) + gross);
      payableByAccount.set(emp.salaryPayableAccountId, (payableByAccount.get(emp.salaryPayableAccountId) || 0) + gross);
      newPayslips.push({
    ...
    const newRun: PayrollRun = {
      ...
      journalEntryId,
      payableBreakdown: Array.from(payableByAccount.entries()).map(([accountId, amount]) => ({ accountId, amount })),
      paid: false,
      createdAt: now,
      createdBy: currentUserName,
    };
  ...
  const confirmPayment = (run: PayrollRun) => {
    ...
    const journalEntryId = `je-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
    const lines = [
      ...run.payableBreakdown.map((b, idx) => ({ id: `l-${journalEntryId}-pay-${idx}`, accountId: b.accountId, debit: b.amount, credit: 0 })),
      { id: `l-${journalEntryId}-cash`, accountId: payingAccountId, debit: 0, credit: run.totalGross },
    ];
    const newEntry: JournalEntry = {
      id: journalEntryId,
      entryNumber: nextEntryNumber(entries),
    ...
    setEntries((prev) => [...prev, newEntry]);
    setRuns((prev) =>
      prev.map((r) => (r.id === run.id ? { ...r, paid: true, paidAt: now, paymentJournalEntryId: journalEntryId, payingAccountId } : r))
    );
    setPayingRunId(null);
  };

  const sortedRuns = useMemo(() => [...runs].sort((a, b) => (a.periodLabel < b.periodLabel ? 1 : -1)), [runs]);
  const viewingRun = runs.find((r) => r.id === viewingRunId) || null;
  const viewingPayslips = viewingRun ? payslips.filter((p) => p.payrollRunId === viewingRun.id) : [];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500">يولّد قسيمة راتب لكل موظف فعّال وقيداً محاسبياً واحداً مجمّعاً عن الفترة المختارة</p>
      </div>
              ...
              <tbody>
                {activeEmployees.map((e) => (
                  <tr key={e.id} className="border-t border-slate-50">
                    ...
                    <td className="px-3 py-2 font-mono">{fmtMoney(allowancesTotal(e.allowances))}</td>
                    <td className="px-3 py-2 font-mono font-semibold">{fmtMoney(grossSalary(e))}</td>
                  </tr>
                ))}
            ...
            <tbody>
              {sortedRuns.map((r) => (
                <tr key={r.id} className="border-b border-slate-50 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-slate-600 whitespace-nowrap">{fmtDateLabel(r.runDate)}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => setViewingRunId(r.id)} className="p-1.5 text-slate-500 hover:text-slate-800 rounded-lg hover:bg-slate-100" title="عرض قسائم الرواتب">
                        <Eye size={14} />
                      </button>
                      {!r.paid && canRecordPayment && (
                        <button onClick={() => openPayment(r)} className="text-xs font-semibold text-emerald-700 hover:bg-emerald-50 px-2 py-1 rounded-lg flex items-center gap-1">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setViewingRunId(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setViewingRunId(null)} className="text-slate-400 hover:text-slate-700">
                <X size={18} />
              </button>
            </div>
            <div className="space-y-2">
                ...
                <div key={p.id} className="border border-slate-100 rounded-xl p-3.5">
                  ...
                  {p.allowances.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {p.allowances.map((a) => (
                        <span key={a.id} className="text-[11px] bg-slate-50 border border-slate-100 rounded-full px-2 py-0.5 text-slate-600">
              ...
              {viewingPayslips.length === 0 && <p className="text-xs text-slate-500 text-center py-4">لا توجد قسائم لهذا التشغيل</p>}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setPayingRunId(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setPayingRunId(null)} className="text-slate-400 hover:text-slate-700">
                <X size={18} />
              </button>
            </div>
            ...
            <p className="text-sm text-slate-600">
              إجمالي المبلغ المطلوب سداده: <span className="font-mono font-bold text-slate-900">{fmtMoney(payingRun.totalGross)}</span>
            </p>
            <div>
              <label className="text-xs font-semibold text-slate-600 mb-1 block">دُفع من حساب</label>
              <select value={payingAccountId} onChange={(e) => setPayingAccountId(e.target.value)} className={inputCls}>
            ...
            <div className="flex gap-2 justify-end pt-2">
              <button onClick={() => setPayingRunId(null)} className="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl">
                إلغاء
              </button>
              <button onClick={() => confirmPayment(payingRun)} className="px-4 py-2 text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-xl">
