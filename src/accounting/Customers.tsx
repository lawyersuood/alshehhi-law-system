...
export default function Customers({
  ...
  const openEdit = (c: Customer) => {
    setError("");
    setForm({ id: c.id, name: c.name, trn: c.trn || "", phone: c.phone || "", email: c.email || "", notes: c.notes || "" });
  };
  ...
  const toggleActive = (c: Customer) => setCustomers((prev) => prev.map((x) => (x.id === c.id ? { ...x, isActive: !x.isActive } : x)));
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">سجل بيانات تواصل العملاء لاستخدامها عند إصدار عروض الأسعار والفواتير — {customers.length} عميل مسجّل</p>
        </div>
        {canManage && (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {customers.map((c) => {
                ...
                return (
                  <tr key={c.id} className={`border-b border-slate-50 last:border-0 ${!c.isActive ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-4 py-2.5 text-xs text-slate-500">{[c.phone, c.email].filter(Boolean).join(" · ") || "—"}</td>
                    <td className="px-4 py-2.5 text-slate-600">{stats?.count || 0}</td>
                    <td className="px-4 py-2.5 font-bold text-slate-800">{fmtMoney(stats?.invoiced || 0)}</td>
                    <td className={`px-4 py-2.5 font-bold ${outstanding > 0 ? "text-[#C5A059]" : "text-slate-400"}`}>{fmtMoney(outstanding)}</td>
                    <td className="px-4 py-2.5">
                      <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold border ${c.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-500 border-slate-200"}`}>
                    ...
                    {(canManage || canDelete) && (
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1 justify-end">
                          {canManage && (
                            <>
                              <button onClick={() => toggleActive(c)} className="text-xs text-slate-500 hover:text-slate-800 px-2 py-1">
                                {c.isActive ? "تعطيل" : "تفعيل"}
                              </button>
                              <button onClick={() => openEdit(c)} className="p-1.5 text-slate-500 hover:text-[#0D382B] rounded-lg hover:bg-[#0D382B]/[0.06]">
                                <Pencil size={14} />
                              </button>
                            </>
                          )}
                          {canDelete && (
                            <button onClick={() => requestDelete(c.id)} className="p-1.5 text-slate-500 hover:text-rose-600 rounded-lg hover:bg-rose-50">
                              <Trash2 size={14} />
                            </button>
                          )}
                        </div>
        ...
        <div className="fixed inset-0 bg-[#08130f]/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setForm(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-900">{form.id ? "تعديل بيانات العميل" : "إضافة عميل جديد"}</h3>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-1">
                  يجب كتابة الاسم بنفس الصيغة المستخدمة في الفواتير حتى تظهر إحصائيات الفوترة بشكل صحيح
                </p>
              </div>
              <div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button onClick={saveForm} className="px-4 py-2 text-sm font-semibold text-white bg-[#0D382B] hover:bg-[#124d40] transition-colors rounded-xl">حفظ</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmDeleteId && deletingCustomer && (
        <div className="fixed inset-0 bg-[#08130f]/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setConfirmDeleteId(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-600">هل أنت متأكد من حذف <span className="font-bold">{deletingCustomer.name}</span>؟</p>
            <div className="flex gap-2 justify-end pt-2">
