      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            سجل موردي وجهات مصروفات المكتب — {vendors.length} مورد مسجّل
          </p>
        </div>
        {canManage && (
          <button
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {vendors.map((v) => (
                <tr
                  key={v.id}
                  className={`border-b border-slate-50 last:border-0 ${!v.isActive ? "opacity-50" : ""}`}
                >
                  ...
                  <td className="px-4 py-2.5">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[11px] font-semibold border ${v.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-500 border-slate-200"}`}
                    >
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-600">
              هل أنت متأكد من حذف <span className="font-bold">{deletingVendor.name}</span>؟
            </p>
            <div className="flex gap-2 justify-end pt-2">
              <button
                onClick={() => setConfirmDeleteId(null)}
