...
export default function PayrollEmployees({
  ...
  const liabilityTypeAccounts = useMemo(
    () =>
      accounts
        .filter((a) => a.isActive && !a.isGroup && a.type === "liability")
        .sort((a, b) => a.code.localeCompare(b.code)),
    [accounts],
  );
  const defaultExpAccount = useMemo(
    () => accounts.find((a) => a.code === SALARY_EXPENSE_ACCOUNT_CODE),
    [accounts],
  );
  const defaultPayableAccount = useMemo(
    () => accounts.find((a) => a.code === SALARY_PAYABLE_ACCOUNT_CODE),
    [accounts],
  );
  ...
  const openNew = () => {
    setError("");
    setForm(
      emptyForm({ expId: defaultExpAccount?.id || "", payableId: defaultPayableAccount?.id || "" }),
    );
  };
  ...
  const updateAllowance = (id: string, patch: Partial<AllowanceItem>) =>
    setForm((f) =>
      f ? { ...f, allowances: f.allowances.map((a) => (a.id === id ? { ...a, ...patch } : a)) } : f,
    );
  const addAllowance = () =>
    setForm((f) => (f ? { ...f, allowances: [...f.allowances, emptyAllowance()] } : f));
  const removeAllowance = (id: string) =>
    setForm((f) => (f ? { ...f, allowances: f.allowances.filter((a) => a.id !== id) } : f));
  ...
  const toggleActive = (e: PayrollEmployee) =>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            سجل مستقل خاص بدورة الرواتب — {employees.length} موظف مسجّل
          </p>
        </div>
        {canManage && (
          <button
      ...
      {(!defaultExpAccount || !defaultPayableAccount) && (
        <div className="flex items-center gap-2 rounded-xl bg-[#0D382B]/[0.05] border border-[#0D382B]/15 px-4 py-3 text-xs text-[#0D382B]">
          ...
          تنبيه: لم يتم العثور على حساب "رواتب وأجور الموظفين" ({SALARY_EXPENSE_ACCOUNT_CODE}) أو
          حساب "رواتب ومستحقات الموظفين" ({SALARY_PAYABLE_ACCOUNT_CODE}) في شجرة الحسابات — يمكن
          اختيار حسابات بديلة يدوياً عند إضافة كل موظف.
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {employees.map((e) => (
                <tr
                  key={e.id}
                  className={`border-b border-slate-50 last:border-0 ${!e.isActive ? "opacity-50" : ""}`}
                >
                  ...
                  <td className="px-4 py-2.5 font-mono">
                    {fmtMoney(grossSalary(e) - e.basicSalary)}
                  </td>
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-900">
                {form.id ? "تعديل بيانات الموظف" : "إضافة موظف جديد"}
              </h3>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, salaryExpenseAccountId: e.target.value })}
                  className={inputCls}
                >
              ...
              <div>
                <label className="text-xs font-semibold text-slate-600 mb-1 block">
                  حساب مستحقات الرواتب
                </label>
                <select
                  value={form.salaryPayableAccountId}
                  onChange={(e) => setForm({ ...form, salaryPayableAccountId: e.target.value })}
                  className={inputCls}
                >
                  <option value="">اختر…</option>
