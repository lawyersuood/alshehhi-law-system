...
export default function Ledger({
  ...
  const rows = useMemo(() => {
    ...
    movements.sort((a, b) =>
      a.date < b.date ? -1 : a.date > b.date ? 1 : a.entryNumber.localeCompare(b.entryNumber),
    );
    let running = 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500">
          كشف حركة القيود المرحّلة على مستوى الحساب الواحد مع الرصيد التراكمي
        </p>
      </div>
                ...
                <tfoot>
                  <tr className="border-t border-slate-200 bg-slate-50/70 font-semibold text-slate-800">
                    ...
                    <td className="px-4 py-2.5 font-mono">
                      {fmtMoney(rows[rows.length - 1]?.balance || 0)}
                    </td>
                  </tr>
                </tfoot>
              )}
