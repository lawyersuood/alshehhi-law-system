...
export default function PurchaseOrders({
  ...
  const sortedOrders = useMemo(
    () =>
      [...orders].sort((a, b) =>
        a.date < b.date ? 1 : a.date > b.date ? -1 : b.orderNumber.localeCompare(a.orderNumber),
      ),
    [orders],
  );
  ...
  const setStatus = (o: PurchaseOrder, status: PurchaseOrderStatus) =>
  ...
  const convertToBill = (o: PurchaseOrder) => {
    ...
    setOrders((prev) =>
      prev.map((x) =>
        x.id === o.id ? { ...x, status: "converted", convertedBillId: newBillId } : x,
      ),
    );
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            طلب شراء أولي للمورد قبل استلام الفاتورة الرسمية — {orders.length} أمر مسجّل
          </p>
        </div>
        {canManage && (
          <button
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-1.5">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-bold ${STATUS_BADGE[o.status]}`}
                    >
