...
app.get("/api/email/settings", requireSupabaseAuth, (req, res) => {
  res.json({
    ...
    settings: {
      ...
      protocol:
        activeEmailConfig.protocol || (activeEmailConfig.port === 465 ? "ssl_tls" : "starttls"),
...
app.post("/api/email/settings", requireSupabaseAuth, (req, res) => {
  const { email, senderName, password, host, port, secure, protocol, rejectUnauthorized } =
    req.body;
  if (!email || !host) {
...
app.post("/api/email/test-connection", requireSupabaseAuth, async (req, res) => {
  ...
  try {
    const { email, password, host, port, secure, protocol, rejectUnauthorized, senderName } =
      req.body;
    ...
    const selectedProtocol =
      protocol || (portToUse === 465 ? "ssl_tls" : protocol === "none" ? "none" : "starttls");
    ...
    res.json({
      ...
      details: {
        ...
        port: portToUse,
        protocol:
          selectedProtocol === "ssl_tls"
            ? "SSL / TLS (التشفير الضمني المباشر)"
            : selectedProtocol === "starttls"
              ? "STARTTLS (الارتقاء المشفر المباشر)"
              : "بدون تشفير (Plain)",
        rejectUnauthorized: Boolean(rejectUnauthorized),
        email: emailToUse,
    ...
    if (
      ...
      recommendation =
        'فشل في توثيق الهوية (Authentication Error): تأكد من صحة البريد الإلكتروني واستخدام "كلمة مرور التطبيقات App Password" المخصصة من حساب Microsoft أو Google.';
    } else if (errCode === "ETIMEDOUT" || errCode === "ESOCKET" || errCode === "ECONNREFUSED") {
      recommendation =
        "تعذر الوصول للمنافذ المحددة (Connection Timeout): تأكد من صحة عنوان الخادم والمنفذ (465 لـ SSL/TLS أو 587 لـ STARTTLS) وحالة جدار الحماية.";
    } else if (
      errMsg.includes("certificate") ||
      errMsg.includes("SELF_SIGNED_CERT_IN_CHAIN") ||
      errCode === "DEPTH_ZERO_SELF_SIGNED_CERT"
    ) {
      recommendation =
        'مشكلة في شهادة الأمان (SSL Certificate Error): يمكنك إلغاء خيار "التحقق الصارم من شهادة SSL" لتجاوز الشهادات المخصصة على خوادم Webmail.';
    } else if (errMsg.includes("WRONG_VERSION_NUMBER") || errMsg.includes("SSL routines")) {
      recommendation =
        "تعارض في بروتوكول الأمان (Protocol Mismatch): تم استخدام SSL/TLS المباشر على منفذ STARTTLS أو العكس. يرجى التبديل بين SSL/TLS و STARTTLS.";
    }
...
app.post("/api/email/send", requireSupabaseAuth, async (req, res) => {
  try {
    ...
    const selectedProtocol =
      configToUse.protocol || (Number(configToUse.port) === 465 ? "ssl_tls" : "starttls");
...
إصدار ومعالجة: ${configToUse.senderName || configToUse.email} | هاتف: +971 50 799 6976
    ...
    res.json({
      ...
      mode: "fallback_saved",
      note:
        "تم حفظ الرسالة في السجل بالرغم من تعذر إرسال SMTP الحقيقي: " +
        (err?.message || "تحقق من كلمة مرور التطبيق"),
    });
  }
});
...
app.post("/api/notifications/send-whatsapp", requireSupabaseAuth, async (req, res) => {
  try {
    ...
    if (!token || !phoneId) {
      res
        .status(500)
        .json({ error: "WHATSAPP_TOKEN أو PHONE_NUMBER_ID غير مُعرّفة في متغيرات بيئة السيرفر" });
      return;
    }
...
app.post("/api/notifications/send-email", requireSupabaseAuth, async (req, res) => {
  try {
    ...
    const emailToUse = hasConfiguredAccount
      ? activeEmailConfig.email
      : envEmail || activeEmailConfig.email;
    const passwordToUse = hasConfiguredAccount ? activeEmailConfig.password : envPassword;
    const hostToUse = hasConfiguredAccount
      ? activeEmailConfig.host
      : envHost || activeEmailConfig.host;
    const portToUse = hasConfiguredAccount
      ? activeEmailConfig.port
      : envPort || activeEmailConfig.port;
    const protocolToUse = hasConfiguredAccount
      ? activeEmailConfig.protocol
      : portToUse === 465
        ? "ssl_tls"
        : "starttls";
...
if (!WHATSAPP_VERIFY_TOKEN) {
  console.warn(
    "تحذير أمني: WHATSAPP_VERIFY_TOKEN غير مُعرّف في متغيرات البيئة — سيُرفض أي تحقق ويب هوك واتساب حتى يُضبط.",
  );
}
...
function getImapConfig() {
  ...
  const hasConfiguredAccount = Boolean(activeEmailConfig.password);
  const email = hasConfiguredAccount
    ? activeEmailConfig.email
    : process.env.SMTP_EMAIL || activeEmailConfig.email;
  ...
  const smtpHost = hasConfiguredAccount
    ? activeEmailConfig.host
    : process.env.SMTP_HOST || activeEmailConfig.host;
...
async function fetchNewInboxEmails(): Promise<{ imported: number; error?: string }> {
  ...
  if (!cfg) {
    return {
      imported: 0,
      error:
        "إعدادات IMAP غير مكتملة (يلزم بريد وكلمة مرور ومضيف IMAP_HOST أو اشتقاقه من SMTP_HOST)",
    };
  }
  ...
  try {
    ...
    if (connectTimedOut) {
      throw new Error(
        "انتهت مهلة الاتصال بخادم IMAP (قد يكون المنفذ 993 محجوباً على هذا الاستضافة)",
      );
    }
...
app.post("/api/notifications/fetch-inbox", requireSupabaseAuth, async (_req, res) => {
  try {
      ...
      new Promise<{ imported: number; error?: string }>((resolve) =>
        setTimeout(
          () =>
            resolve({
              imported: 0,
              error: "انتهت مهلة الاتصال بخادم البريد (IMAP) — يرجى المحاولة لاحقاً",
            }),
          25000,
        ),
      ),
    ...
    res
      .status(500)
      .json({ success: false, error: err?.message || "فشل غير متوقع أثناء جلب البريد الوارد" });
  }
});
...
const appSpecSchema = {
  ...
  properties: {
    ...
    description: {
      type: Type.STRING,
      description: "Comprehensive 2-3 sentence overview of what the application achieves",
    },
    archetype: {
      type: Type.STRING,
      description:
        "Category e.g. Productivity & Wellness, Creator Tool, SaaS Dashboard, FinTech, E-Commerce, Education",
    },
    ...
    visualDesign: {
      ...
      properties: {
        ...
        surfaceColor: {
          type: Type.STRING,
          description: "Surface card background hex e.g. #ffffff",
        },
        textColor: { type: Type.STRING, description: "Text main color e.g. #0f172a" },
        fontFamily: {
          type: Type.STRING,
          description:
            "Font family suggestion e.g. Plus Jakarta Sans, Inter, Outfit, Space Grotesk",
        },
        borderRadius: { type: Type.STRING, description: "Border radius style e.g. 12px or 16px" },
        styleName: {
          type: Type.STRING,
          description: "Aesthetic theme name e.g. Clean Bio Teal, Cyber Dark, Minimal Slate",
        },
      },
      required: [
        "theme",
    ...
    dataSchema: {
      ...
      items: {
        ...
        properties: {
          ...
          sampleRecordsJson: {
            type: Type.STRING,
            description: "JSON string array of 3-4 realistic sample data objects",
          },
        },
        ...
        tabs: {
          ...
          items: {
            ...
            properties: {
              ...
              icon: {
                type: Type.STRING,
                description: "Lucide icon name e.g. CheckSquare, Sparkles, BarChart2, Package",
              },
              description: { type: Type.STRING },
            },
        ...
        actions: {
          ...
          items: {
            ...
            properties: {
              ...
              type: {
                type: Type.STRING,
                description: '"add", "filter", "toggle", "delete", "ai_generate"',
              },
              targetEntity: { type: Type.STRING },
              fieldsJson: {
                type: Type.STRING,
                description: "JSON string array of field definitions",
              },
            },
...
In codeFiles, provide clean, idiomatic, fully formed React TypeScript components and Express routes.`;
    ...
    const processedSpec = {
      ...
      interactiveApp: {
        tabs: rawResult.interactiveApp?.tabs || [
          { id: "main", name: "Main View", icon: "Layout", description: "Primary Dashboard" },
        ],
      ...
      if (docType === "poa") {
        return res.json({
          ...
          extracted: {
            ...
            expiryDate: new Date(Date.now() + 365 * 2 * 24 * 60 * 60 * 1000)
              ...
              .split("T")[0],
            scope:
              "تمثيل ومرافعة أمام محاكم دبي والاتحادية، فتح البلاغات، تقديم المذكرات والطعون، الصلح والإقرار.",
        ...
        return res.json({
          ...
          extracted: {
            ...
            installmentsNotes:
              "دفعة أولى 15,000 درهم عند التوقيع + دفعة 15,000 عند الجلسة الأولى + 15,000 عند الحكم.",
